...
class LeaveBalanceAdjustment(models.Model):
    ...
    delta_days = models.DecimalField(
        max_digits=5,
        decimal_places=2,
        help_text="Use + to grant, − to deduct.",
    )
...
def leave_year_stats(user, year: int | None = None, as_of: date | None = None) -> dict:
    ...
    Accrual policy:
      ...
      - Plus/minus admin adjustments for that year
    ...
    # All leave requests of this user in selected year
    qs = (
    ...
    approved_days = round(float(approved_days), 2)
    rejected_days = round(float(rejected_days), 2)
    cancelled_days = round(float(cancelled_days), 2)
    pending_days = round(float(pending_days), 2)

    remaining = max(0.0, round(accrued - approved_days, 2))
