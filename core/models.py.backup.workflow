...
class Profile(models.Model):
    ...
    # Admin can change this value for each employee.
...
class Request(models.Model):
    ...
    # Workflow:
    # - If required_user is set, that specific user must act first.
    # - After replacement approves: MIDDLE -> EXECUTIVE -> BOARD.
    ...
    def submit(self) -> None:
        """
        DRAFT -> SUBMITTED.
        Order: Replacement if present -> MIDDLE -> EXECUTIVE -> BOARD.
        """
...
def can_user_approve(user, req: Request) -> bool:
    """
    Replacement -> Middle same department -> Executive -> Board.
    """
    if req.is_terminal():
        return False
...
def apply_decision(user, req: Request, decision: str, comment: str = "") -> None:
    ...
    # ---------- Role progression ----------
    # IMPORTANT: Check if this was a replacement (specific user) approval
    if req.required_user_id:
        # After replacement approves, move to role-based approval starting with MIDDLE
        req.required_user = None
    ...
    # Role-based progression (no specific user required)
    if req.required_role == Role.MIDDLE:
        # Middle approved → next approver must be Executive
        req.required_role = Role.EXECUTIVE
        req.required_user = None
        req.status = RequestStatus.IN_REVIEW
        req.save(update_fields=["required_role", "required_user", "status"])
        return
    ...
    if req.required_role == Role.EXECUTIVE:
        # Executive approves → next approver is Board
...
class LeaveRequest(models.Model):
    ...
    def requested_days(self) -> float:
        """
        DAILY: prefer duration_days, else inclusive date count.
...
class LeaveBalanceAdjustment(models.Model):
    ...
    Positive = grant extra days.
    Negative = deduct days.

    This is applied on top of Profile.annual_leave_days.
    """
...
def _clamp_days_to_year(start: date, end: date, year: int) -> float:
    """
    Clamp [start, end] inclusive to the given year.
    """
...
def _leave_days_in_year(lr: LeaveRequest, year: int) -> float:
    """
    Calculate how many leave days this leave request counts for in the selected year.
    """
...
def leave_year_stats(user, year: int | None = None, as_of: date | None = None) -> dict:
    ...
    if profile:
        # Start from annual allowance
        annual_allowance = float(profile.annual_leave_days or 30.0)

        # If admin manually set approved/rejected/cancelled days, use them
        ...
        # Live calculation for pending or new requests
        qs = (
